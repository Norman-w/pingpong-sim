...
- 画面规格：1920×1080、30 fps、75.4 秒。
- 竖版规格：1080×1920、30 fps、75.4 秒；从同一真实三维母版重新取景，保留球网、落点和双球。
...
| 0:31–0:40 | 高有效摩擦录屏 | 第二跳后红球跨过零，变成上旋 |
...
- 标准条件蓝球第二跳后约 -1070 rpm；临界红球约 -23 rpm；高有效摩擦红球约 +126 rpm。RPM 是仿真计算值。
- 音频轨道为单独验证过的 AAC，75.4 秒；采样检查有非零音量，成片同时包含视频轨和音频轨。
- 媒体全部位于仓库外 `/Users/norman/Movies/pingpong-sim/spin-reversal/`，不进入 Git。

## 当前限制

ChatCut 在当前环境不可调用，因此没有声称使用 ChatCut。v09 使用本机女声、SRT 和 Swift/AVFoundation 合成；如需在 ChatCut 中继续轻剪，只需导入上述母版、旁白或 SRT，不要替换三维原始画面。发布仍由用户手动完成。
