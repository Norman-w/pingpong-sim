...
### 跟球视线与身位（已校准：错过 A 后必须继续跟到 B）

**禁止**：把「到了第一个建议击球点 / 错过窗口 A」当成跟球结束，立刻 `look-back` 回看发球机，然后再发现还有窗口 B 才把视线甩回来——会造成相机抖动，也不符合真实回合。

**领域理由**：

- 球还在我方、尚未被我方打回去 → 对方不可能已经回球，**没有必要先着急看对面**
- 窗口 B 仍属于**同一来球**的合法处理：我方落台弹起后那条抛物线的**下降段**，落到小朋友够得着的合适高度再出手
- 小朋友主观判断起跳点（窗口 A）打不了时：
  - **不会**往前冲去够近网高点（近网球时冲过去也够不着）
  - 而是**往后撤**，视线继续贴着弹起弧线（近台→远台），边跟边等下降到可处理高度再出手
- **只有出手之后**，才回看对面对手 / 发球机

**实现要点（接入现有跟球状态机时必须遵守）**：

1. lob 且已判定窗口 A 错过、窗口 B 仍未触球、球未二次落台/坠地：  
   - 相位保持（或强制）`follow-contact`（持续跟球）  
   - **不得**进入 `look-back` / 不得把 `trackingTarget` 甩向发球机
2. 自动身位：窗口 A 错过后目标深度应随弧线**后退**（等下降点），不是钉在近台起跳位
3. `post-contact-source`（击球完毕回看出球点）**仅在实际触球 / 本球处理结束之后**才允许

```text
落台 → 窗口A(起跳下压) —错过→ 继续跟弧后退 → 窗口B(下降够得到)出手 → 这时才回看发球机
```

## 接入

1. lob 跟踪：落台后维护窗口 A 状态机；结束且未触球 → 闪红 + 钉住错过标记 + 青点切 B  
2. 同上状态下**视线与身位继续跟到窗口 B**；禁止提前 look-back  
3. 技术集 / 色带：可第二期；第一期以红边 + 双标记 + 连续跟球为主

## 刻意不做（第一期）

- 非 lob、改 bounce 规则、完整人体关节/挥拍力矩、多点实战触球
- 为「教学好看」而发明第二落台/地板弹起等非真实窗口
